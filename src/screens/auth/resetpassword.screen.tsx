...
export default function ResetPasswordScreen({
  ...
  const [actionResponseType, setActionResponseType] = useState<
    AlertModalType | undefined
  >(undefined);
  ...
  const doResetPassword = async () => {
    const req = await newResetPassword({
  ...
  const doUpdatePassword = async () => {
    const req = await newUpdatePassword({
      email: user.email || "",
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        <ScreenTitle
          title={!userLoggedIn ? `Enter New Password` : `Change Password`}
          intro={
            !userLoggedIn
      ...
      <AlertModal
        ...
        title={!userLoggedIn ? `New Password Set` : `Password Changed`}
