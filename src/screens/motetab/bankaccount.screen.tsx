...
export default function BankDetailsScreen({
  ...
  const getBanks = async () => {
    ...
    if (!banks?.hasError) {
      const allBanks = banks.data.message.map(
        (elem: { bankName: string; bankCode: string }) => {
          return {
            label: elem.bankName,
            value: elem.bankCode,
          };
  ...
  const handleVerify = async () => {
    ...
    switch (actionType) {
      ...
      case "submit":
        ...
        message = `${
          request?.message || request?.statusText || request?.error
        }`;
        // navigation.goBack()
        break;
      default:
    ...
    if (success) {
      showToast({
        ...
        type: "success",
        message: `Account ${
          actionType === "submit" ? "added" : "updated"
        } successfully`,
      });
      dispatch(
