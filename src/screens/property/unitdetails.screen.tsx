...
export default function UnitDetailsScreen({
  ...
  const doDeleteUnit = () => {
    showConfirm({
      message: oneUnit.occupyingStatus
        ? `This unit is occupied, are you sure you want to delete?`
        : `Are you sure you want to delete unit - ${oneUnit.unitName}`,
      title: `Delete Unit`,
      type: `delete`,
      onConfirm: async () => {
        deleteOneUnit({
              ...
              message:
                res.data?.hasError === false
                  ...
                  : res?.data?.message ||
                    res?.message ||
                    res?.error ||
                    `Unknown error occured deleting unit`,
              title: `Delete Unit`,
  ...
  return (
    <ScrollView>
      <Layout
        ...
        rightComponent={
          deleting ? (
            <ActivityIndicator color={colorsConstants.colorDanger} />
          ) : (
            <TouchableOpacity
        ...
        <View style={styles.container}>
          ...
          <View>
              ...
              {[
                {
                  id: 1,
                  label: "Unit Rent:",
                  value: `${currencySymbol["ngn"]}${
                    oneUnit.unitRent === null
                      ? "0.00"
                      : formatCurrency(Number(oneUnit.unitRent))
                  }`,
                  valueTextOpacity: 1,
                },
                {
                  ...
                  value: `${currencySymbol["ngn"]}${formatCurrency(
                    Number(oneUnit.unitServiceCharge)
                  )}`,
                  valueTextOpacity: 1,
                },
                {
                  ...
                  label: "Legal Fee:",
                  value: `${currencySymbol["ngn"]}${
                    oneUnit.unitLegalCharge === null
                      ? "0.00"
                      : formatCurrency(Number(oneUnit.unitLegalCharge))
                  }`,
                  valueTextOpacity: 1,
                },
                {
                  ...
                  value: `${currencySymbol["ngn"]}${
                    oneUnit.unitAgreementCharge === null
                      ? "0.00"
                      : formatCurrency(Number(oneUnit.unitAgreementCharge))
                  }`,
                  valueTextOpacity: 1,
                },
                {
                  ...
                  value: `${currencySymbol["ngn"]}${
                    oneUnit.unitCommissionCharge === null
                      ? "0.00"
                      : formatCurrency(Number(oneUnit.unitCommissionCharge))
                  }`,
                  valueTextOpacity: 1,
                },
                {
                  ...
                  label: "Other Charges:",
                  value: `${currencySymbol["ngn"]}${
                    oneUnit.unitOtherCharges === null
                      ? "0.00"
                      : formatCurrency(Number(oneUnit.unitOtherCharges))
                  }`,
                  valueTextOpacity: 1,
                },
                {
            ...
            {tenantData?.email && (
              ...
              >
                <Image
                  source={require("src/assets/images/icons/human-icon.png")}
                  style={{
                    width: 20,
                    height: 20,
                  }}
            ...
            <DefaultButton
              ...
              onPress={() => {
                if (oneUnit.occupyingStatus) {
                  Alert.alert(
                    `Hold On!`,
                    `Are you sure you want to remove this tenant from unit?\nAction is permanent`,
                    [
                      {
                        text: `Cancel`,
                      },
                  ...
                  if (!user?.bankAvailable) {
                    Alert.alert(
                      `Hold On!`,
                      `You have not added your bank detail.\nWant to add it now?`,
                      [
                        {
                          text: `Not now`,
                          onPress: doAddTenant,
