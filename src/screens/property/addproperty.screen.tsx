...
export default function AddPropertyScreen({
  ...
  const { loading, createProperty, newCreateProperty, created, editProperty } =
    useProperty();
  ...
  React.useEffect(() => {
    const backAction = () => {
      if (!showAddNewBtn) {
        Alert.alert(
          `Hold On!`,
          `You haven't added any unit to created property`,
          [
            {
              text: `Cancel`,
            },
  ...
  const doCreateProperty = async () => {
    ...
    if (!req?.hasError) {
      ...
      showToast({
        title: "Add Property",
        type: "success",
        message: req?.data?.statusMessage || "Property created successfully",
      });
    ...
    if (
      req?.errorType === "subscription" ||
      req?.data?.errorType === "subscription"
    )
      showConfirm({
  ...
  //       data: {
  //         "email": "ayowalexy+00@gmail.com",
  //         "occupationalStatus": "",
  //         "propertyLocation": "Yiy",
  //         "propertyName": "Hhu",
  //         "propertyState": "Cross River"
  //     },
  //     method: 'post',
  //     headers: {
  //       Authorization:
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <ScreenTitle
          title={`${
            route.params?.actionType === "edit" ? "Edit" : "Add"
          } Property`}
          containerStyle={{
